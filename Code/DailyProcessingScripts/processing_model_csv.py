...
df_listingAddress = df['listingAddress']

df.drop(columns=['listingAddress'], inplace=True)


#code for prediction of model
...
if all(column in df.columns for column in expected_columns):
    ...
    df['PredictedListingRent'] = df['PredictedListingRent'].round().astype(int)


df['listingAddress'] = df_listingAddress
...
#code for pushing it back to s3
csv_buffer = StringIO()
df.to_csv(csv_buffer, index=False)

object_name = 'modelOutput/'+today_str+'.csv'
